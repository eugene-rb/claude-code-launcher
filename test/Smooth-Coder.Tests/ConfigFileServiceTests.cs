...
public class ConfigFileServiceTests
{
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), "SmoothCoderTests_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "SmoothCoderTests_" + Guid.NewGuid().ToString("N"), "CLAUDE.md");
