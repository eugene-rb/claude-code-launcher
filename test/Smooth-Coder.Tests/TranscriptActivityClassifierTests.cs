...
public class TranscriptActivityClassifierTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "SmoothCoderTests_" + Guid.NewGuid().ToString("N") + ".jsonl");
    ...
    {
        var file = Path.Combine(Path.GetTempPath(), "SmoothCoderTests_missing_" + Guid.NewGuid().ToString("N") + ".jsonl");
