...
public class SkillFolderScannerTests
{
    ...
    {
        var missing = Path.Combine(Path.GetTempPath(), "SmoothCoderTests_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "SmoothCoderTests_" + Guid.NewGuid().ToString("N"));
