...
{
    private readonly string _dir = Path.Combine(Path.GetTempPath(), "ClaudeLauncherStatusLineTests_" + Guid.NewGuid().ToString("N"));
    ...
    public void Install_IntoExistingSettings_AddsStatusLineAndKeepsUnrelatedKeys()
    {
        ...
        var stashed = installer.Install(@"C:\apps\ClaudeLauncher.App.exe");
    ...
    public void Install_NoSettingsFile_CreatesOne()
    {
        ...
        Assert.Empty(Directory.GetFiles(_dir, "*.bak-claudelauncher-*"));
    }
    ...
    public void Install_WithExistingStatusLine_StashesItAndBacksUp()
    {
        ...
        Assert.Single(Directory.GetFiles(_dir, "*.bak-claudelauncher-*"));
    }
